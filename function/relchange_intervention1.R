VCrelchange_intervention1<- function(HBI_mean, pi_samples, parous_mean, lambda_samples,
  ...
  ## intervention
    ...
    PA = 1 - A0
    pi=pi_samples
    kappa=lambda_samples*pi_samples
    xi=xi_samples
    omega=omega_samples
    PA1 = A0*Pf*chi*eta*P_B_b*P_C_b/(P_B_b*P_C_b*(chi*P_D_b*P_E_b + (1-chi)*P_D_b*P_E_b)*eta*P_B_b*P_C_b)
    PAh = A0*Pf*(1-chi)/(eta*P_B_b*P_C_b*(chi*P_D_b*P_E_b +(1-chi)*P_D_b*P_E_b))
    ...
    alpha_H = c(alpha_H_b, alpha_A_b, (1-pi)*alpha_H_b, kappa*alpha_H_b)
    P_C = c(P_C_b, P_C_b,(1-xi)*P_C_b,omega)
    PA = exp(-(sum(alpha_H*N) + mu_A_b)*theta_d)
    PAi = (1 - PA)*((alpha_H*N)/(sum(alpha_H*N) + mu_A_b))
