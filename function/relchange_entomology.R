VCrelchange_entomology <- function(HBI_mean, HBI_samples, parous_mean, parous_samples,
  ...
  ## samples
  RelativeChange = rep(NA, length(N_samples))
  c = 0
  for(sample in 1:N_samples){
    c = c + 1
    chi = HBI_samples[sample]
    Pf = parous_samples[sample]
    A0 = sac_samples[sample]
    tau = tau_samples[sample]
    PA = 1 - A0
    PA1 = A0*Pf*chi*eta*P_B_b*P_C_b/(P_B_b*P_C_b*(chi*P_D_b*P_E_b + (1-chi)*P_D_b*P_E_b)*eta*P_B_b*P_C_b)
    PAh = A0*Pf*(1-chi)/(eta*P_B_b*P_C_b*(chi*P_D_b*P_E_b +(1-chi)*P_D_b*P_E_b))
    PA2 = eta*PAh
    ...
    RelativeChange[c] = (other-baseline)/baseline
  }
