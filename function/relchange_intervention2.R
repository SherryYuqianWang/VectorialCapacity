VCrelchange_intervention2<- function(HBI_mean, pi_samples, parous_mean, betam_samples,betad_samples,
                        sac_mean, xi_samples, tau_mean, rho_samples,
                        N_b){
  theta_d = 8/24
  P_B_b = 0.95
  P_C_b = 0.95
  ...
  ## intervention
    ...
    A0 = sac_mean
    tau = tau_mean
 RelativeChange = rep(0, length(N_samples))
  for(sample in 1:N_samples){
    PA = 1 - A0
    pi=pi_samples[sample]
    betam=betam_samples[sample]
    betad=betad_samples[sample]
    xi=xi_samples[sample]
    rho=rho_samples[sample]
    PA1 = A0*Pf*chi*eta*P_B_b*P_C_b/(P_B_b*P_C_b*(chi*P_D_b*P_E_b + (1-chi)*P_D_b*P_E_b)*eta*P_B_b*P_C_b)
    PAh = A0*Pf*(1-chi)/(eta*P_B_b*P_C_b*(chi*P_D_b*P_E_b +(1-chi)*P_D_b*P_E_b))
    ...
    #Pdf = PAi*P_B_b*P_C_b*P_D_b*P_E_b
    MalariaHost = c(1,0,1,0)
    Hosts = which(MalariaHost == 1)
    N = c((1-c)*N_b, N_b, c*N_b, c*N_b)
    alpha_H = c(alpha_H_b, alpha_A_b, (1-pi)*alpha_H_b, (betad+betam)*alpha_H_b)
    P_C = c(P_C_b, P_C_b,(1-xi)*P_C_b,betad/(betad+betam))
    mu_A = mu_A_b+rho*alpha_H_b*c*N_b
    PA = exp(-(sum(alpha_H*N) + mu_A)*theta_d)
    PAi = (1 - PA)*((alpha_H*N)/(sum(alpha_H*N) + mu_A))
    ...
    RelativeChange[sample] = (other-baseline)/baseline
    }
