...
N <- 100000
params <- c("HBI","parous","sac","tau","thetad","thetas","N")
order <- "fourth"
R<-100
type <- "percent"
conf <- 0.95
...
X[, 5] <- qunif(X[,5],min = theta_d_min, max = theta_d_max)
X[, 6] <- floor(qunif(X[,6],min = theta_s_min, max = theta_s_max+1))
X[, 7] <- floor(qunif(X[,7],min = N_b_min, max = N_b_max+1))
...
for (s in 1:nrow(X1)){
  y[s] <- with(X1[s,], VCrelchange_entomology_GSA(HBI_mean, HBI, parous_mean, parous, 
                                                  sac_mean, sac, tau_mean, tau, thetad, thetas,
                                                  N))
}
...
saveRDS(plot,file="~/malaria/VC/output/ent_sobol_new")
saveRDS(ind.dummy,file="~/malaria/VC/output/ent_dummy_new")
...
ggsave("dirus_ento_sobol.png", width = 8, height = 7)
