...
saveRDS(DT_plot,file="~/malaria/VC/output/int_sac_sobol")


p<-ggplot(DT_plot[order=="Si"],aes(x=sac, y=mean,col=parameter))+
...
p<-ggplot(DT_plot[order=="Ti"],aes(x=sac, y=mean,col=parameter))+
  #geom_rect(aes(xmin=min(HBI_range), xmax=max(HBI_range), ymin=0, ymax=0.05), fill="grey", color=NA,alpha=0.05) +
  geom_line()+
  geom_ribbon(aes(ymin=low.ci,ymax=high.ci,fill=parameter),alpha=0.2,colour = NA)+
  ylim(0,1)+
...
ggsave("all_first_int2_sac.png", width = 8, height = 7)
ggsave("all_total_int2_sac.png", width = 8, height = 7)



###LHS

sobol<-data.frame(matrix(ncol = 6, nrow = n_params*length(HBI_range)*length(coverage)))
#sobol_plot<-list()
#p<-list()
#only use for loop
for (c in 1:3){
