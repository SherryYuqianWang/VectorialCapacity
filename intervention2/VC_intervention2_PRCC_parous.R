...
save(DT_plot,file = "~/malaria/VC/output/int_parous_PRCC")


#plot
p<-ggplot(DT_plot,aes(x=parous, y=mean,color=parameter))+
  ...
  #annotate(geom="text", x=0.3, y=0.5, label="betad", size=3.5, color="#F8766D")+
  #annotate(geom="text", x=0.3, y=-1, label="betam/", size=3.5, color="#A3A500")+
  #annotate(geom="text", x=0.3, y=-0.7, label="pi", size=3.5, color="#00BF7D")+
  #annotate(geom="text", x=0.37, y=-1, label="rho", size=3.5, color="#00B0F6")+
  #annotate(geom="text", x=0.3, y=-0.5, label="xi", size=3.5, color="#E76BF3")+
...
ggsave("all_prcc_int2_parous.png", width = 8, height = 7)
