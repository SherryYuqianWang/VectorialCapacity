...
Sys.time()
saveRDS(DT_plot,file="~/malaria/VC/output/int_HBI_sobol")


p<-ggplot(DT_plot[order=="Si"],aes(x=HBI, y=mean,col=parameter))+
...
p<-ggplot(DT_plot[order=="Ti"],aes(x=HBI, y=mean,col=parameter))+
  #geom_rect(aes(xmin=min(HBI_range), xmax=max(HBI_range), ymin=0, ymax=0.05), fill="grey", color=NA,alpha=0.05) +
  geom_line()+
  geom_ribbon(aes(ymin=low.ci,ymax=high.ci,fill=parameter),alpha=0.2,colour = NA)+
  ylim(0,1.06)+
...
gsave("all_first_int2_HBI.png", width = 8, height = 7)
ggsave("all_total_int2_HBI.png", width = 8, height = 7)
